...
const SolutionDetailPage = () => {
  ...
  useEffect(() => {
    const fetchSolution = async () => {
      if (!slug) {
        setError('Solution slug is required');
        setLoading(false);
        return;
      }
      ...
      try {
        setLoading(true);
        const solutionData: any = await productsApi.getSolutionBySlug(slug);                                    
    ...
    <>
      <div className={`pt-20 min-h-screen bg-gray-50 ${isRTL ? 'rtl' : 'ltr'}`}>
        <div className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 ${isRTL ? 'text-right' : 'text-left'}`}>
          ...
          {/* Header with Image */}
          <div className='w-full flex flex-col items-center justify-center py-4 mb-8'>                
            {imageUrl ? (
              <img 
                className='object-cover w-full h-96 rounded-xl mb-6'
                src={imageUrl} 
                alt={solution.title} 
              />
            ) : (
              <div className="w-full h-96 bg-gradient-to-br from-blue-100 to-blue-200 flex items-center justify-center rounded-xl mb-6">
                <span className="text-blue-600 font-semibold text-2xl">{solution.title}</span>
              </div>
            )}
            <h1 className='w-full text-center my-4 text-4xl font-bold text-black'>{solution.title}</h1>
            {solution.description && (
              <p className='w-full text-center text-xl text-gray-600 max-w-3xl mx-auto'>
                {solution.description}
              </p>
            )}
          </div>
