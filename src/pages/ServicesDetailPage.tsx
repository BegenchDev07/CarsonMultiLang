...
const ServicesDetailPage = () => {
  ...
  const [product, setProduct] = useState<ServiceType | null>(null);
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      try {
        setLoading(true);
        const productData = await productsApi.getService(id);        
  ...
  const renderGallery = (data:any,alt:string) => {    
    if(isMobile){      
      return (
          ...
          data.map((image:any, index:number) => {
              return (
                <button
                  ...
                  className={`bg-white rounded-xl p-1 shadow-lg transition-all duration-300 w-1/3 flex-shrink-0 ${
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
            ...
            <div className='h-auto'>
              <div className="bg-white rounded-2xl p-4 shadow-lg mb-4">
                {allImages.length > 0 ? (
                  <img
                    src={`https://api.skyelectronica.com${product.display_image?.formats?.large?.url}`}
                    alt={product.name}
                    loading="lazy"
                    decoding="async"
                    width="600"
              ...
              {allImages.length > 1 && (
                renderGallery(allImages,product.name)
              )}
            </div>
              ...
              <div className="bg-white rounded-2xl p-8 shadow-lg">
                  <div className="flex items-center justify-between mb-4">
                    <span className="bg-blue-100 text-blue-800 text-sm font-medium px-3 py-1 rounded-full">
                      {product.service_cateogry?.name || t('common.product')}
                    </span>
